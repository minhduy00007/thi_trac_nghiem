...
<script>
        ...
        if (file) {
            if (file.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || file.type === 'application/vnd.ms-excel') {
                ...
                reader.onload = function (e) {
                    ...
                    axios.post(secureUrl("{{ route('admin.quan-ly.sinh-vien.handle-them-sinh-vien') }}"), { data: students })
    ...
    document.getElementById('download-link').addEventListener('click', function() {
        axios.get(secureUrl("{{ route('admin.quan-ly.sinh-vien.download-template-sinh-vien') }}"), { responseType: 'blob' })
            .then(function(response) {
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'sinh_vien_template.xlsx');
