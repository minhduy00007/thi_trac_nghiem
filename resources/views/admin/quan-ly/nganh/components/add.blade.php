...
<script>
        ...
        if (file) {
            if (file.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || file.type === 'application/vnd.ms-excel') {
                ...
                reader.onload = function (e) {
                    ...
                    // Xử lý dữ liệu
                    const nganhs = jsonData.slice(1).map(row => ({
                        ma_nganh: row[0],
                        ten_nganh: row[1],
                        ma_khoa: row[2],
                    }));
                    // Gửi dữ liệu lên máy chủ
                    axios.post(secureUrl("{{ route('admin.quan-ly.nganh.handle-them-nganh') }}"), { data: nganhs })
    ...
    document.getElementById('download-link').addEventListener('click', function() {
        axios.get(secureUrl("{{ route('admin.quan-ly.nganh.download-template-nganh') }}"), { responseType: 'blob' })
            .then(function(response) {
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'nganh_template.xlsx');
