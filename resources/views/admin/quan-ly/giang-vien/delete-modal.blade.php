<div class="fixed top-0 left-0 inset-0 overflow-y-auto bg-gray-600 bg-opacity-50 w-screen h-screen" style="display: none; z-index:100;" id="modal-xoa-giang-vien">
    <div class="bg-white rounded p-4 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        <div class="modal-title mb-6">
          <input type="hidden" id="data-id">
        </div>
        <form id="form-xoa-giang-vien" class="p-6 mb-0">
            ...
            <div class="text-3xl text-red-400 flex items-center justify-center font-semibold">
              Bạn chắc chắn muốn xóa giảng viên này?
            </div>
            <div class="flex justify-end mt-8">
                <button onclick="xoa()" type="submit" class="mr-3 border border-emerald-400 py-2 px-4 mt-4 rounded inline-flex items-center hover:bg-emerald-500 font-bold hover:text-white">
                Xác nhận
              </button>
              <button onclick="tatModal()" class="mr-3 border border-rose-400 py-2 px-4 mt-4 rounded inline-flex items-center hover:bg-rose-500 font-bold hover:text-white">
                Hủy
              </button>
            </div>
        </form>
