<div class="flex items-center">
    ...
    <button  onclick="showModalThem('{{ $modalThem }}')"  class=" mr-2 border border-cyan-400 py-2 px-4 rounded inline-flex items-center hover:bg-cyan-500 font-bold hover:text-white">
...
<script>
        ...
        if (file) {
            if (file.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || file.type === 'application/vnd.ms-excel') {
                ...
                reader.onload = function (e) {
                    ...
                    // Xử lý dữ liệu
                    const teachers = jsonData.slice(1).map(row => ({
                        ma_giang_vien: row[0],
                        ten_giang_vien: row[1],
                        so_dien_thoai: row[2],
                        email: row[3],
                        ngay_sinh: row[4],
                        ma_khoa: row[5],
                    ...
                    axios.post(secureUrl("{{ route('admin.quan-ly.giang-vien.handle-them-giang-vien') }}"), { data: teachers })
    ...
    document.getElementById('download-link').addEventListener('click', function() {
        axios.get(secureUrl("{{ route('admin.quan-ly.giang-vien.download-template-giang-vien') }}"), { responseType: 'blob' })
            .then(function(response) {
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'giang_vien_template.xlsx');
