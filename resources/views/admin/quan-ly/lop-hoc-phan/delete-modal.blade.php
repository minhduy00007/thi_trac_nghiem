<div class="fixed top-0 left-0 inset-0 overflow-y-auto bg-gray-600 bg-opacity-50 w-screen h-screen" style="display: none; z-index:100;" id="modal-xoa-lop-hoc-phan">
    <div class="bg-white rounded p-4 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        <div class="modal-title mb-6">
          <input type="hidden" id="data-id">
        </div>
        <form id="form-xoa-lop-hoc-phan" class="p-6">
            ...
            <div class="text-3xl text-red-400 flex items-center justify-center font-semibold">
              Bạn chắc chắn muốn xóa lóp học phần này?
            </div>
            <div class="flex justify-end mt-8">
              ...
              <button onclick="tatModal()" class="mr-3 border-2 border-rose-500 py-2 px-4 rounded inline-flex items-center hover:bg-rose-500 font-bold hover:text-white">
                Hủy
              </button>
            </div>
        </form>
...
<script type="text/javascript">
  ...
  function xoa() {
    $('#form-xoa-lop-hoc-phan').on('submit', function(event){
        event.preventDefault();
        axios.post(secureUrl("{{ route('admin.quan-ly.lop-hoc-phan.handle-xoa-lop-hoc-phan') }}"), {
