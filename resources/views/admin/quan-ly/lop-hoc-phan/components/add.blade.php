<div class="flex items-center">
    ...
    <button  onclick="showModalThem('{{ $modalThem }}')"  class="mr-2 border border-cyan-600 py-2 px-4 rounded inline-flex items-center hover:bg-cyan-400 font-bold hover:text-white">
...
<script>
        ...
        if (file) {
            if (file.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || file.type === 'application/vnd.ms-excel') {
                ...
                reader.onload = function (e) {
                    ...
                    // Xử lý dữ liệu
                    const lopHocPhans = jsonData.slice(1).map(row => {
                        const danhSachSinhVien = row[5] ? row[5].split(', ').map(ma_sv => ({ ma_sinh_vien: ma_sv })) : [];
                        const danhSachGiangVien = row[6] ? row[6].split(', ').map(ma_gv => ({ ma_giang_vien: ma_gv })) : [];
                        const danhSachBaiThi = row[7] ? row[7].split(', ').map(ma_bt => ({ ma_bai_thi: ma_bt })) : [];

                        return {
                            ma_lop_hoc_phan: row[0],
                            ten_lop_hoc_phan: row[1],
                            ma_mon_hoc: row[2],
                            thoi_gian_bat_dau: row[3],
                            thoi_gian_ket_thuc: row[4],
                            danh_sach_sinh_vien: danhSachSinhVien,
                            danh_sach_giang_vien: danhSachGiangVien,
                            danh_sach_bai_thi: danhSachBaiThi,
                        };
                    });
                    ...
                    axios.post(secureUrl("{{ route('admin.quan-ly.lop-hoc-phan.handle-them-lop-hoc-phan') }}"), { data: lopHocPhans })
    ...
    document.getElementById('download-link').addEventListener('click', function() {
        axios.get(secureUrl("{{ route('admin.quan-ly.lop-hoc-phan.download-template-lop-hoc-phan') }}"), { responseType: 'blob' })
            .then(function(response) {
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'lop_hoc_phan_template.xlsx');
