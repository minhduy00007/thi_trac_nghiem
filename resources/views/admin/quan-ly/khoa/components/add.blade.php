...
<script>
        ...
        if (file) {
            if (file.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || file.type === 'application/vnd.ms-excel') {
                ...
                reader.onload = function (e) {
                    ...
                    // Xử lý dữ liệu
                    const khoas = jsonData.slice(1).map(row => ({
                        ma_khoa: row[0],
                        ten_khoa: row[1],
                    }));
                    // Gửi dữ liệu lên máy chủ
                    axios.post(secureUrl("{{ route('admin.quan-ly.khoa.handle-them-khoa') }}"), { data: khoas })
    ...
    document.getElementById('download-link').addEventListener('click', function() {
        axios.get(secureUrl("{{ route('admin.quan-ly.khoa.download-template-khoa') }}"), { responseType: 'blob' })
            .then(function(response) {
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'khoa_template.xlsx');
